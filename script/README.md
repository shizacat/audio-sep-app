...
В GitHub Actions то же экспорт делает одно задание, и только когда отправлен git-тег. Оно клонирует AudioSep, скачивает чекпоинты и ставит зависимости из `script/requirements-ci.txt` в отдельное окружение бегуна. Готовые файлы передаются сборкам пакетов. Локальный запуск по-прежнему использует окружение AudioSplit и `script/requirements.txt`.
