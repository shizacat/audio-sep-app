...
- Окно отделяет звук в фоне через ONNX-модели. Аудиофайл читается и сохраняется. При разработке модели лежат в `local/`, у собранного приложения macOS — в каталоге `models` рядом с `AudioSep.app`.
